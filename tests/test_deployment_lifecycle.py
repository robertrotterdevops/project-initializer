...
class TestValidateConfigScript(unittest.TestCase):
    """Task 2 TDD tests: validate-config.sh"""
    ...
    def test_v1_flux_returns_validate_config_key(self):
        """Test 1: main() with gitops_tool=flux returns dict containing validate-config.sh"""
        files = main(self.project_name, self.description, self.flux_context_eck)
        self.assertIn("scripts/validate-config.sh", files)

    def test_v2_validate_has_find_kustomization_yaml(self):
        """Test 2: validate-config.sh contains a find command for kustomization.yaml files"""
        files = main(self.project_name, self.description, self.flux_context_eck)
        content = files["scripts/validate-config.sh"]
        self.assertIn("find", content)
        self.assertIn("kustomization.yaml", content)

    def test_v3_validate_has_yaml_syntax_check(self):
        """Test 3: validate-config.sh contains YAML syntax validation with python3 and yaml.safe_load"""
        files = main(self.project_name, self.description, self.flux_context_eck)
        content = files["scripts/validate-config.sh"]
        self.assertIn("python3", content)
        self.assertIn("yaml.safe_load", content)

    def test_v4_validate_has_dangling_reference_check(self):
        """Test 4: validate-config.sh contains dangling reference check logic"""
        files = main(self.project_name, self.description, self.flux_context_eck)
        content = files["scripts/validate-config.sh"]
        self.assertIn("Dangling reference", content)

    def test_v5_validate_checks_required_directories(self):
    ...
    def test_v6_validate_exits_one_on_failure(self):
    ...
    def test_v7_validate_exits_zero_on_success(self):
        """Test 7: validate-config.sh exits 0 when all checks pass"""
        files = main(self.project_name, self.description, self.flux_context_eck)
        content = files["scripts/validate-config.sh"]
        self.assertIn("exit 0", content)

    def test_v8_total_scripts_is_seven_with_eck(self):
        """Test 8: Total scripts generated by main() with eck_enabled=True is 7"""
        files = main(self.project_name, self.description, self.flux_context_eck)
