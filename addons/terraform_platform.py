...
class TerraformPlatformGenerator:
    ...
    def _root_main(self) -> str:
        ...
        if self.platform == "proxmox":
            proxmox_args = (
                ...
                "  proxmox_enable_cloud_init_growpart = var.proxmox_enable_cloud_init_growpart\n"
    ...
    def _root_variables(self) -> str:
        ...
        return (
            ...
            'variable "proxmox_enable_cloud_init_growpart" {\n  type = bool\n  default = false\n}\n'
            'variable "vmid_start" {\n  type = number\n  default = 5000\n}\n'
    ...
    def _module_proxmox_main(self) -> str:
        return (
            ...
            'variable "proxmox_snippets_storage" { type = string }\n'
            'variable "proxmox_enable_cloud_init_growpart" { type = bool }\n'
            ...
            '}\n\n'
            'resource "proxmox_virtual_environment_file" "cloud_init_growpart" {\n'
            '  count        = var.proxmox_enable_cloud_init_growpart ? 1 : 0\n'
            '  content_type = "snippets"\n'
            '  datastore_id = var.proxmox_snippets_storage\n'
            '  node_name    = var.proxmox_node_name\n\n'
            '  source_raw {\n'
            '    file_name = "${var.cluster_name}-grow-root.yaml"\n'
            '    data      = <<-EOF\n'
            '      #cloud-config\n'
            '      runcmd:\n'
            '        - |\n'
            ...
            '          PARTNUM="$(lsblk -no PARTN "$ROOT_SRC" 2>/dev/null || true)"\n'
            '          [ -n "$PKNAME" ] && [ -n "$PARTNUM" ] || exit 0\n'
            '          DISK="/dev/$PKNAME"\n'
            ...
            '          case "$FSTYPE" in\n'
            '            ext2|ext3|ext4) command -v resize2fs >/dev/null 2>&1 && resize2fs "$ROOT_SRC" || true ;;\n'
            '            xfs) command -v xfs_growfs >/dev/null 2>&1 && xfs_growfs / || true ;;\n'
            '          esac\n'
            '    EOF\n'
            '  }\n'
            '}\n\n'
    ...
    def _proxmox_tfvars(self) -> str:
        ...
        return (
            ...
            'proxmox_enable_cloud_init_growpart = false\n'
            'vmid_start = 5000\n'
            'vm_disk_size_gb = 80\n'
