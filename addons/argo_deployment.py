...
class ArgoDeploymentGenerator:
    ...
    def generate(self) -> Dict[str, str]:
        ...
        for component, path, sync_wave, ignore_diff in self._component_apps():
            files[f"argocd/apps/components/{component}.yaml"] = self._generate_component_app(
                component=component,
                path=path,
                sync_wave=sync_wave,
                ignore_differences=ignore_diff,
            )
    ...
    def _component_apps(self) -> List[Tuple[str, str, str, str]]:
        ...
        if self.enable_otel_collector:
            components.append(("observability", "observability", "4", """  ignoreDifferences:
  - group: ""
    kind: Secret
...
"""
    ...
    def _generate_component_app(self, component: str, path: str, sync_wave: str, ignore_differences: str = "") -> str:
        ignore_block = f"\n{ignore_differences}\n" if ignore_differences else ""
        return f"""apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
...
spec:
  ...
  destination:
    server: "https://kubernetes.default.svc"
    namespace: {self.project_name}{ignore_block}
  syncPolicy:
    automated:
      prune: true
      selfHeal: true
